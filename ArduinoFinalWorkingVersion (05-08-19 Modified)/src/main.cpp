...
int param_tonecsintensity = 127; // default to max intensity, Olivia splitting tone intensity variable out for ease of use 6/4/2020
int param_changecsint = 1; // default to change the cs intensity, Olivia added this so that Matlab has to send a value of 1 to move the digital potentiometer wiper within session. Preserves wiper function for longer.
...
void setup() {
  ...
  SPI.begin(); // Olivia uncommented 6/4/2020 to permit tone usage again
  SPI.setBitOrder(MSBFIRST);  // if you are using the MCP4131 (tone digital potentiometer)
...
void configureTrial() {
   ...
   if (param_tonecsintensity > 127) { param_tonecsintensity = 127;} // Olivia splitting tone intensity variable out for ease of use 6/4/2020
   if (param_csch == ch_tone && param_changecsint == 1) { //Alvaro 05/09/19 sheiney committed on Jan 18, 2017; Olivia update 6/4/2020
     setDiPoValue(param_tonecsintensity); //Alvaro 05/09/19 sheiney committed on Jan 18, 2017; Olivia update 6/4/2020
